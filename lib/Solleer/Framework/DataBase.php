...
class Database {
    ...
    public function getDBInfo($part) {
        if ($part === 'dsn') {
            ...
            // If it is Online
            else if ($this->environment->getDebug())
                return $this->defaultDSN . ($this->settings->onlineDBPrefix ?? "") . ($this->settings->testDBPrefix ?? "") . $name;
            else return $this->defaultDSN . ($this->settings->onlineDBPrefix ?? "") . $name;
        }
